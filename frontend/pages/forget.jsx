...
import { sessionVerficiation, usernameStatus, OtpProvider, resetPasswordApi } from '@/redux/api/redux.user'
...
export default function SignUp() {
    ...
    onSubmit: async (values) => {
      ...
      switch (result) {
        case 200:
          ...
          break;
        case 404:
          form.setFieldError('username', 'Username not founded')
          break;
        case 500:
          router.push('/500')
          break;
  ...
  return (
    ...
    >
      <Head>
        <title>TailCode | Forget</title>
        <meta name='description' content='Recover account and code 2x faster' />
      </Head>
      <div className='col-span-3 lg:col-span-1 px-6 py-2 flex flex-col space-y-8 relative'>
        <Logo />
        <form className='space-y-6' onSubmit={form.handleSubmit}>
          <legend className='font-bold text-2xl text-black dark:text-white'>Forget Password</legend>
        ...
        {otp && <OTP values={form.values} api={resetPasswordApi} handleback={() => setotp(false)} />}
      </div>
