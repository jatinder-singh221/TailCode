  ...
  return (
    <>
      ...
      <main className="w-[95%] lg:w-[80%] min-h-screen mx-auto mt-14" id='top'>
        ...
        <Search state={search} handleClose={() => setsearch(!search)} url='/components/list' />
        {!loading ?
          <ul className="w-full  animate-show-out mb-5 grid grid-cols-4 gap-5 pt-4">
            {component.length > 0 ? component.map((data, index) => {
              return <List key={index} {...data} />
            }) : <li className='w-full h-screen flex py-10 col-span-4'>
              <div className='w-full space-y-10'>
                <h1 className='text-center text-xl text-black font-medium dark:text-white'>No Components Founded</h1>
